...
const Carousel = (props) => {
	...
	const next = () =>
		curr < props.gallery.length - 1 ? setCurr(++curr) : console.log("end");
	...
	for (let i = 0; i < props.gallery.length; i++) {
		gallery.push(
			...
			<span
				...
				onClick={curr === i ? "" : curr + 1 === i ? next : prev}
			>
	...
	return (
		<div className="carousel">
			{gallery}
			<i
				className="fas fa-caret-square-left fa-3x button-left"
				onClick={prev}
			></i>
			<i
				className="fas fa-caret-square-right fa-3x button-right"
				onClick={next}
			></i>
		</div>
	);
